...
public class NumberOfDays {
	...
	int getNumberOfDays(int fromDay, int fromMonth, int fromYear, int toDay, int toMonth) {
		...
		if (fromMonth == toMonth) {
			numberOfDays = getDays(fromDay, toDay);
		} else {
			numberOfDays = getDays(fromDay, getLastDay(fromMonth, fromYear));
			for (int i = fromMonth + 1; i < toMonth; i++) {
				numberOfDays += getLastDay(i, fromYear);
			}
			numberOfDays += toDay;
		}
		return numberOfDays;
	...
	int getDays(int fromDay, int toDay) {
